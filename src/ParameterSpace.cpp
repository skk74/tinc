...
void ParameterSpace::loadFromNetCDF(std::string ncFile) {
  ...
  // Get main group ids
  if (nc_inq_grp_ncid(ncid, "internal_states", &internal_state_grpid) == 0) {
    ...
    for (int i = 0; i < num_state_grps; i++) {
      ...
      if ((retval = nc_inq_var(state_grp_ids[i], varid, nullptr, &xtypep,
    ...
    std::cout << "No group 'internal_states' in " << ncFile << std::endl;
  }

  if (nc_inq_grp_ncid(ncid, "parameters", &parameters_grpid) == 0) {
  ...
  if (nc_inq_grp_ncid(ncid, "conditions", &conditions_grpid) == 0) {
